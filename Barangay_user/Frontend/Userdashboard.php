...
if (isset($_SESSION['resident_id'])) {
  ...
  $sqlUser = "SELECT * FROM residents WHERE username = '$username'";
}
    ...
    <div class="requestcard"> 
        ...
        if($result && $result->num_rows > 0){
            while($row = $result->fetch_assoc()){
                ...
                echo '    <p style="margin:5px 0 0 0; font-size:13px; color:#64748b;"><i class="fa-regular fa-calendar"></i> Submitted: ' . date("M d, Y h:i A", strtotime($row['submitted_at'])) . '</p>';

                if (!empty($row['approved_at'])) {
                    echo '    <p style="margin:5px 0 0 0; font-size:13px; color:#64748b;"><i class="fa-regular fa-clock"></i> Approved: ' . date("M d, Y h:i A", strtotime($row['approved_at'])) . '</p>';
                }
